...
<html lang=s>
		...
		String sq3 = "SELECT * FROM book_circle_discussion WHERE cdis_did = '"+did+"'";
		...
		String cid = rs3.getString("cdis_cid");
		
		String sq4 = "SELECT * FROM book_circle WHERE crcl_cid = '"+cid+"'";
        pst4 = c1.prepareStatement(sq4);
		rs4 = pst4.executeQuery();
		rs4.next();
		
		String sq5 = "SELECT * FROM book_circle_discussion_comment WHERE cdic_did = '"+did+"' ORDER BY cdic_time DESC";
		...
		while(rs5.next()){
			litComments.add(rs5.getString("cdic_text"));
			litWriter.add(rs5.getString("cdic_uid"));
			litComTime.add(rs5.getString("cdic_time"));
		}
...
<div id="myModal7" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-body">
		<form action="addcirccomment.jsp?&fn=<%=rs1.getString("user_uid")%>&did=<%= did %>" method="post" >
...
<div class="jumbotron">
		<h2>Cirkel: <a href="cirdisc.jsp?cid=<%=cid%>"> <%= rs4.getString("crcl_name")%></a></h2>
		<h3>Tråd: <%= rs3.getString("cdis_topic")%></h3>
</div> 
