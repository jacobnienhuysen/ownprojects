        ...
        java.sql.PreparedStatement pst1,pst2,pst3;
		...
		if(rs1.next()){ //om det inmatade användarnamnet finns i registret
			if(!rs1.getString("user_user_name").equals(uName)){
				...
				usid = rs1.getString("user_uid");
				
				String sq2 ="SELECT * FROM book_user_admin WHERE admin_uid= ?";
				pst2 = c1.prepareStatement(sq2);
				pst2.setString(1, usid);
				rs2 = pst2.executeQuery();

				if(rs2.next() && rs2.getString("admin_adm").equals("1")){
					String sq3 ="SELECT * FROM book_user_md5 WHERE md5_uid= ?";
					pst3 = c1.prepareStatement(sq3);
					pst3.setString(1, usid);
					rs3 = pst3.executeQuery();
					
					if(rs3.next()){ //om det finns två likadana användarid i båda tabellerna
						...
						if(check.equals(rs3.getString("md5_pass"))){
							
							Cookie user = new Cookie("user",usid);
							user.setMaxAge(3600*24);
							response.addCookie(user); 
							response.sendRedirect("adminMain.jsp"); //logged-in page
